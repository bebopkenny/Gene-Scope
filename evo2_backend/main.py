...
app = modal.App("predictor_evo2", image=evo2_image)

volume = modal.Volume.from_name("hf_cache", create_if_missing=True)
mount_path = "/root/.cache/huggingface"

# Function runs on the cloud
@app.function(gpu="H100", volumes={mount_path: volume}, timeout=1000)
def run_brca1_predictor():
    import base64
    ...
    brca1_subset = pd.read_excel(
    '/evo2/notebooks/brca1/41586_2018_461_MOESM3_ESM.xlsx',
    header=2,
    )
    brca1_subset = brca1_subset[[
        'chromosome', 'position (hg19)', 'reference', 'alt', 'function.score.mean', 'func.class',
    ]]

    brca1_subset.rename(columns={
        'chromosome': 'chrom',
        'position (hg19)': 'pos',
        'reference': 'ref',
        'alt': 'alt',
    ...
    brca1_subset['class'] = brca1_subset['class'].replace(['FUNC', 'INT'], 'FUNC/INT')
    ...
    brca1_subset = brca1_subset.iloc[:500].copy()
    ...
    # DEBUG TEST
    print("Threshold:", confidence_params['threshold'])
    print("LOF STD:", confidence_params['lof_std'])
    print("FUNC STD:", confidence_params['func_std'])
...
def brca1_example():
    ...
    print("Running BRCA variant analysis with evo2")

    # Run inference 
    result = run_brca1_predictor.remote()
    # DEBUG TEST 
    print("Returned confidence params:", result.get('confidence_params'))
    plot_data = None
...
def analyze_variant(relative_pos_in_window, reference, alternative, window_seq, model):
    variant_sequence = window_seq[:relative_pos_in_window] + \
        alternative + window_seq[relative_pos_in_window+1:]
    
    ref_score = model.score_sequences([window_seq])[0]
    var_score = model.score_sequences([variant_sequence])[0]
...
class Evo2Model:
    ...
    def analyze_single_variant(self, variant_position: int, alternative: str, genome: str, chromosome: str):
        ...
        print(f"Variant alternative: {alternative}")
